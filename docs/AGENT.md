...
The POSIX agent, which covers Linux and macOS, and the PowerShell one carry the
same version number and are released together, so "this machine is on 1.13.0"
means the same thing whichever of the three platforms it is running on. A fix to one is a release of both, even when the other needed
